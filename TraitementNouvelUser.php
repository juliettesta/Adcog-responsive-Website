...
while ($ligne = $resultat -> fetch())
{
    ...
    {
        $LoginUtilise=1; //Le login est déjà utilisé
    }
}
...
$Requete= $BDD -> prepare("SELECT * FROM ADRESSE WHERE adresse_voie= ?");
    ...
    //
        ...
        //Clé étrangère :  on regarde si l'adresse existe
    ...
    $requete = $BDD -> prepare ("INSERT INTO UTILISATEUR(utilisateur_nom, utilisateur_prenom, utilisateur_dateNaissance, utilisateur_dateInscription, utilisateur_statut,  utilisateur_mail, utilisateur_tel, utilisateur_mdp, utilisateur_login, adresse_id, utilisateur_valide)
                                    VALUES (:nomU, :prenomU, :dateneeU, NOW(), :statutU, :mailU, :telU, :mdpU, :loginU, :adresse_idU, 0)");
    ...
    $_SESSION['UtilisateurAjoute']=true; // L'utilisateur a été ajouté
    
    //Changement de page
    header("Location: connexion.php?mdp=".$_POST['mdpU']."&login=".$_POST['loginU']);
    
} //Fin du if si aucune erreur
else { // Sinon on reste sur la même page avec les champs remplis
