...
//Fonctions permettant d'enregistrer les images et fichiers
...
if (is_uploaded_file($_FILES['logoE']['tmp_name'])) { //Pour une image
...
if (is_uploaded_file($_FILES['fichierO']['tmp_name'])) { // pour un fichier
...
//aucune restriction
    //Entreprise
if (($_SESSION['erreurIext'] == true) || ($_SESSION['erreurItaille'] == true)) //Si extension ou taille de l'image non autorisé
{ $_SESSION['erreurImage']=true ;}
else {$_SESSION['erreurImage']=false ;}

if (($_SESSION['erreurFext'] == true) || ($_SESSION['erreurFtaille'] == true)) //Si extension ou taille du fichier non autorisé
...
{ $_SESSION['erreurdateDO']=true;}
    ...
    //mdp
if ((strlen ($mdpO) < 4)&&(strlen ($mdpO) != 0)) // mdp de caractères minimum 
{ $_SESSION['erreurmdpO'] = true ;}
else {$_SESSION['erreurmdpO'] = false ;}


//Ajout dans la BDD s'il n'y a pas d'erreur
if (($_SESSION['erreurdateDO']==false) && ($_SESSION['erreurImage'] ==false) && ($_SESSION['erreurFichier']==false)&&($_SESSION['erreurmdpO']==false)) { // dans le if mettre des && == false pour toutes les variables
    ...
    //Si l'adresse n'existe pas, il faut la rajouter dans la BDD avant de pouvoir update l'offre
    ...
    // Entreprise
    //
    //Si l'entreprise n'existe pas, il faut la rajouter dans la BDD avant de pouvoir update l'offre
    //On part du principe qu'une entreprise avec le même nom et le même site internet sont identiques
    ...
    //Si le contact n'existe pas, il faut la rajouter dans la BDD avant de pouvoir update l'offre  
    ...
    //
    // S'il y a pas de clé étrangère pour l'adresse, on n'ajoute rien à cette table de la BDD
    ...
    if ($mdpO == "") {
        $requete  = $BDD -> prepare ("UPDATE OFFRE
                                  SET offre_nom= :nomO, offre_profil= :profilO, offre_dateDeb= :dateDO, offre_duree= :dureeO, offre_descrCourte= :descripCO, offre_descrLongue= :descripLO, offre_fichier= :fichierO, offre_valide = 1, offre_signalee = 0, offre_renum= :remunO, contact_id= $contact_id, entreprise_id= $entreprise_id, adresse_id= :adresse_idU
                                  WHERE offre_id=".$_SESSION['offre_idIA']);
    ...
    $requete -> bindvalue('fichierO', $fichierO, PDO::PARAM_STR); 
    ...
    $requete -> execute();
        ...
        //Cryptage du nouveau mdp
         $mdpO=password_hash($mdpO,PASSWORD_DEFAULT);
            ...
            $requete  = $BDD -> prepare ("UPDATE OFFRE
                                  SET offre_nom= :nomO, offre_mdp= :mdpO, offre_profil= :profilO, offre_dateDeb= :dateDO, offre_duree= :dureeO, offre_descrCourte= :descripCO, offre_descrLongue= :descripLO, offre_fichier= :fichierO, offre_valide = 1, offre_signalee = 0, offre_renum= :remunO, contact_id= $contact_id, entreprise_id= $entreprise_id, adresse_id= :adresse_idU
                                  WHERE offre_id=".$_SESSION['offre_idIA']);
    ...
    $_SESSION['IAOffreModifiee']=true; // Offre modifiée
    
    //Changement de page
    header("Location: IAOffres.php");

} //Fin du if s'il y a aucune erreur
// S'il y a une erreur, on reste sur la page du formulaire
else {
    header('Location: IAModifierOffre.php');
