    ...
    <body>
            ...
            <div class="enBasADroite margeDroite">
                <div class="btn-group ">
                  ...
                  <ul class="dropdown-menu">
                    <li><a href="panierSave.php?tri=recent">+ récent</a></li>
                    <li><a href="panierSave.php?tri=ancien">+ ancien</a></li>
                    <li role="separator" class="divider"></li>
                    <li><a href="panierSave.php?tri=az">A-Z</a></li>
                    <li role="separator" class="divider"></li>
                    <li><a href="panierSave.php?tri=commenceBientot">Commence bientôt</a></li>
                  </ul>
                </div>
            </div>
        ...
        function AfficherOffre($offre_id,$postule,$liaison_id,$save,$bool)
            {
                ...
                //affichage de l'adresse si elle existe
                     if (isset($adresse['adresse_ville'])){
        ...
        <nav  class="navbar navbar-default gris" role="navigation">
             <ul class="nav navbar-nav">
                 <li><a href="panierSave.php" class="navCouleurContainer">Annonces sauvegardées</a></li>
                 <li><a href="panierPost.php" class="navPrincipale">Annonces postulées</a></li>
             </ul>
        </nav>
        <br/>';
        //code qui permet de récupérer toutes les offre_id du panier
        require("db/connect.php");
        if(isset($_SESSION['id']))
        {
            ...
            { 
                $requete='SELECT * FROM liaison_panier,offre WHERE utilisateur_id='.$_SESSION['id'].' AND offre_valide=1 '
                        . 'AND offre_signalee=0 AND liaison_panier.offre_id=offre.offre_id AND liaisonPanier_save=1 ORDER BY offre_datePoste DESC';
            }
            //Tri par +ancien
            if (isset($_GET['tri']) && $_GET['tri']=="ancien")
            { 
                $requete='SELECT * FROM liaison_panier,offre WHERE utilisateur_id='.$_SESSION['id'].' AND offre_valide=1 '
                        . 'AND offre_signalee=0 AND liaison_panier.offre_id=offre.offre_id AND liaisonPanier_save=1 ORDER BY offre_datePoste';
            }
            //Tri par A-Z
            if (isset($_GET['tri']) && $_GET['tri']=="az")
            { 
                $requete='SELECT * FROM liaison_panier,offre WHERE utilisateur_id='.$_SESSION['id'].' AND offre_valide=1 '
                        . 'AND offre_signalee=0 AND liaison_panier.offre_id=offre.offre_id AND liaisonPanier_save=1 ORDER BY offre_nom '; 
            }
            //Tri par commence au plus tôt commenceBientot
             if (isset($_GET['tri']) && $_GET['tri']=="commenceBientot")
            { 
                $requete='SELECT * FROM liaison_panier,offre WHERE utilisateur_id='.$_SESSION['id'].' AND offre_valide=1 '
                . 'AND offre_signalee=0 AND liaison_panier.offre_id=offre.offre_id AND liaisonPanier_save=1 ORDER BY offre_dateDeb DESC';

            }    
                $resultat=$BDD -> query($requete);
                if ($resultat->rowCount()==0)
                {
                    print 'Vous n\'avez sauvegardé aucune offre pour le moment !<br/>';
                }
                else {
